<template>
    <div class="bg-white border border-mun-gray-200 rounded-lg p-6">
        ...
        <form @submit.prevent="submitPositionPaper">
            ...
            <!-- Editor Toolbar -->
            <div class="mb-4">
                <div class="flex items-center space-x-2 p-2 border border-mun-gray-300 rounded-t-lg bg-mun-gray-50">
                    <button type="button" @click="editor?.chain().focus().toggleBold().run()"
                        :class="{ 'bg-mun-blue text-white': editor?.isActive('bold') }"
                        class="p-1 rounded hover:bg-mun-gray-200">
                        <strong>B</strong>
                    </button>
                    <button type="button" @click="editor?.chain().focus().toggleItalic().run()"
                        :class="{ 'bg-mun-blue text-white': editor?.isActive('italic') }"
                        class="p-1 rounded hover:bg-mun-gray-200">
                        <em>I</em>
                    </button>
                    <button type="button" @click="editor?.chain().focus().toggleUnderline().run()"
                        :class="{ 'bg-mun-blue text-white': editor?.isActive('underline') }"
                        class="p-1 rounded hover:bg-mun-gray-200">
                        <u>U</u>
                    </button>
                    <div class="w-px h-6 bg-mun-gray-300"></div>
                    <button type="button" @click="editor?.chain().focus().toggleBulletList().run()"
                        :class="{ 'bg-mun-blue text-white': editor?.isActive('bulletList') }"
                        class="p-1 rounded hover:bg-mun-gray-200">
                        •
                    </button>
                    <button type="button" @click="editor?.chain().focus().toggleOrderedList().run()"
                        :class="{ 'bg-mun-blue text-white': editor?.isActive('orderedList') }"
                        class="p-1 rounded hover:bg-mun-gray-200">
                        1.
                    </button>
                    <div class="w-px h-6 bg-mun-gray-300"></div>
                    <button type="button" @click="insertSection('background')"
                        class="px-2 py-1 text-xs rounded hover:bg-mun-gray-200">
                        + Background
                    </button>
                    <button type="button" @click="insertSection('position')"
                        class="px-2 py-1 text-xs rounded hover:bg-mun-gray-200">
                        + Position
                    </button>
                    <button type="button" @click="insertSection('solutions')"
                        class="px-2 py-1 text-xs rounded hover:bg-mun-gray-200">
                        + Solutions
                    </button>
                    <button type="button" @click="insertSection('conclusion')"
                        class="px-2 py-1 text-xs rounded hover:bg-mun-gray-200">
                        + Conclusion
                    </button>
                </div>
            ...
            <!-- Tiptap Editor -->
            <div class="border border-mun-gray-300 rounded-b-lg">
                <div ref="editorElement"
                    class="min-h-[500px] p-4 prose prose-lg max-w-none focus-within:ring-1 focus-within:ring-mun-blue"
                    :class="{ 'border-mun-red-300': errors.content }"></div>
            </div>
...
const wordCount = computed(() => {
    return textContent.value.split(/\s+/).filter(word => word.length > 0).length
})
...
const insertSection = (sectionType) => {
    const sectionTemplates = {
        background: {
            title: 'Background and Context',
            content: 'Describe the historical background, current situation, and why this topic is important to your country...'
        },
        position: {
            title: `${props.delegate.countryName}'s Position`,
            content: 'Explain your country\'s official position, policies, and interests regarding this topic...'
        },
        solutions: {
            title: 'Proposed Solutions and Recommendations',
            content: 'Propose specific solutions, recommendations, and actions that your country supports...'
        },
        conclusion: {
            title: 'Conclusion',
            content: 'Summarize your main points and emphasize your country\'s commitment to finding solutions...'
        }
    }

    const section = sectionTemplates[sectionType]
    if (!section || !editor.value) return
...
const validateForm = () => {
    ...
    if (textContent.value.trim().length < 50) {
        errors.value.content = 'Please provide substantial content for your position paper'
    }
...
const submitPositionPaper = async () => {
    ...
    try {
        ...
        const response = await apiMethods.documents.submitPositionPaperText(submissionData)
        ...
        const errorMessage = error.response?.data?.error || error.message || 'Failed to submit position paper'
...
const loadDraft = () => {
    try {
        ...
        if (savedDraft) {
            ...
            if (draftData.content && editor.value) {
...
:deep(.ProseMirror h2) {
    @apply text-lg font-semibold text-mun-gray-900 mt-6 mb-3;
}
