<template>
    <div class="min-h-screen bg-gradient-mun">
        ...
        <!-- Main layout -->
        <template v-else>
            <UniversalSidebar :sidebar-collapsed="sidebarCollapsed" user-role="delegate" :user="authStore.user"
                ...
                <div class="bg-white border-b border-mun-gray-200 px-6 py-3">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-3 min-w-0">
                            <span
                                class="px-2 py-0.5 text-xs font-medium rounded-full bg-blue-100 text-blue-700">Delegate</span>
                            <span v-if="ctx.participation.value?.country?.name"
                                class="text-sm font-medium text-mun-gray-900">
                                {{ ctx.participation.value.country.name }}
                            </span>
                            <span class="text-mun-gray-300">·</span>
                            <span class="text-sm text-mun-gray-600 truncate">
...
// Build nav paths relative to /session/:committeeId/delegate
const base = computed(() => ctx.basePath.value + '/delegate')
